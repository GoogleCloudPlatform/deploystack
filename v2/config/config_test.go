...
func TestReadConfig(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"custom_options": {
			path: "dsfolders/customs_options",
			want: Stack{
				Config: Config{
					Title:         "TESTCONFIG",
					Description:   "A test string for usage with this stuff.",
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			testdata := filepath.Join(testFilesDir, tc.path)
			err := s.FindAndReadRequired(testdata)
...
func TestNewConfigReport(t *testing.T) {
	testdata := filepath.Join(testFilesDir, "configs/multi")
