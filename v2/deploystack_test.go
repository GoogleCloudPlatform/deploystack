...
func TestPrecheckMulti(t *testing.T) {
	...
	if os.Getenv("BE_CRASHER") == "1" {
		...
		testdata := filepath.Join(testFilesDir, "configs")
		path := filepath.Join(testdata, "multi")
		oldWD, _ := os.Getwd()
...
func TestCheckForContact(t *testing.T) {
	tests := map[string]struct {
		...
		"basic": {
			in: filepath.Join(testFilesDir, "contact/contact.yaml"),
			want: gcloud.ContactData{
				AllContacts: gcloud.DomainRegistrarContact{
					Email: "test@example.com",
					Phone: "+155555551212",
...
func TestInit(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"no_custom": {
			path: filepath.Join(testFilesDir, "dsfolders/no_customs"),
			want: config.Stack{
		...
		"no_name": {
			path: filepath.Join(testFilesDir, "dsfolders/no_name"),
			want: config.Stack{
				Config: config.Config{
					Title:         "NONAME",
					Name:          "",
		...
		"custom": {
			path: filepath.Join(testFilesDir, "dsfolders/customs"),
			want: config.Stack{
				Config: config.Config{
					Title:         "TESTCONFIG",
					Name:          "test",
		...
		"custom_options": {
			path: filepath.Join(testFilesDir, "dsfolders/customs_options"),
			want: config.Stack{
				Config: config.Config{
					Title:         "TESTCONFIG",
					Name:          "test",
